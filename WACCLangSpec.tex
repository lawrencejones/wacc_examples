...
Variable names must not clash with other variable names within the same scope.

They must be at least one character in length, the first character of which must be either an underscore or lower/uppercase characters from 'a' to 'z'. The first character is followed by zero or more characters from a selection of underscore and alphanumerical.
...
A variable must be declared before it is referenced, where referencing is the use of the variable in any operation.

Any attempt to access an undeclared variable results in a compile error, detectable at compile time.
...
If scope is determined by S, where S' is a scope defined within scope S (and so on), then declaration of a variable with ident I in S' will take precedence over any declaration of the same name I within S. Hence the variable to be used in the case of a duplicate name is that declared in the deepest scope.

Once the child scope is exited, the variable is destroyed and the rules apply again for scoping.
...
An exit statement signifies the termination of the current program execution. Given an expression as an argument, the program will cease to execute and control flow is handed back to the governing process. Any lines of code following the exit statement will not be executed, and the given argument is returned as the program exit code.
...
The behaviour of the print statements for each type of expression is shown in Table, along with some example cases.
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{C|L|C|C}
    ...
    \hline
    \lit*{string} or \lit*{char[]} & Outputs the contents of the string (char array) to stdout. & "hello" & "hello" \\
    \hline
    Other Array Types & Will print the pointer value in hexidecimal & [ 0, 0 ] & 0xff26ed10 \\  
    \hline
    \lit*{pair} & Prints the pointer value of the pair & \lit*{newpair(a, b)}\footnotemark[1] & 0x12028 \\
    \hline
  \end{tabulary}
...
A while loop is a program construct that allows repetition of a body of code up to the point at which the initial boolean condition evaluates to false. The \lit{while} keyword is followed with an expression that evaluates to a \lit{bool}. On the start of each iteration, the expression is recalculated and assuming it evaluates to \lit{true}, then the body statement is executed. At any point if the expression is evaluated and it turns to \lit{false} then the body statement is skipped and execution continues past the while loops scope (the end of which is denoted by the \lit{done;} token).
...
A tool using the \lit{;} operator to control the order of evaluation of ultiple statements within a program. It allows us to create a single expression out a series of expressions.
...
Side effect free expressions do not alter the programs state; the programs state remains unchanged after processing the expression.
...
\begin{table}
  ...
  \begin{tabular}{cclc}
    ...
    \lit*{\char`\\ n} & \lit*{0x0a} & newline & LF \\
    \lit*{\char`\\ f} & \lit*{0x0c} & new page & FF \\
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{CCCL}
    ...
    \lit{ord} & \lit*{int} & \lit*{char} & Character value \\
    \lit{toInt} & \lit*{char} & \lit*{int} & Integer value \\
    \hline
  \end{tabulary}
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{CCCCCL}
    ...
    \lit{>} & 3 & \lit*{int} & \lit*{int} & \lit*{bool} & Greater Than \\
    \lit{>=} & 3 & \lit*{int} & \lit*{int} & \lit*{bool} & Greater Than or Equal \\
    \lit{<} & 3 & \lit*{int} & \lit*{int} & \lit*{bool} & Less Than \\
    \lit{<=} & 3 & \lit*{int} & \lit*{int} & \lit*{bool} & Less Than or Equal \\
    \lit{==} & 4 & \lit*{int/bool} & \lit*{int/bool} & \lit*{bool} & Equality \\
    \lit{!=} & 4 & \lit*{int/bool} & \lit*{int/bool} & \lit*{bool} & Inequality \\
...
A comment is a line of code ignored by the compiler. It is preceded by a hash and followed by any number of other characters except eol. \lit{eol} signifies the end of the comment.
